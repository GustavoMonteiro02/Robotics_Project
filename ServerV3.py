...
# ----- CONFIG -----
ROBO_IP = '192.168.137.29'
ROBO_PORT = 8080
STREAM_PORT = 9090
ROBO_PORT = 9050
ROBO_PORT_ALERT = 9040
...
sock_alert.bind(('0.0.0.0', ROBO_PORT_ALERT))
sock_stream.settimeout(0.2)
sock_alert.settimeout(0.2)
...
<body>
  ...
  <div class="dpad">
    <div></div>
    <button onclick="sendCmd(8)">&#8593;</button>
    <div></div>
  ...
  <div class="speed-control">
    ...
    <span id="speedValue">50</span><br>
    <button class="send-speed" onclick="sendSpeed()">Change Speed</button>
  </div>

  <!-- Alerta -->
  <div id="alertBox">Awaiting Alerts...</div>
...
def get_alert():
    global latest_alert
    with alert_lock:
        response = make_response(latest_alert or "Sem alertas")
...
def alert_receiver():
    ...
    while True:
        try:
            data, _ = sock_alert.recvfrom(1024)
            alert = data.decode('utf-8').strip()
            global latest_alert
            with alert_lock:
                latest_alert = alert
...
if __name__ == "__main__":
    ...
    threading.Thread(target=alert_receiver, daemon=True).start()
