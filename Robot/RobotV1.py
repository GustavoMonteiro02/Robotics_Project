...
v_speed_socket_port = 9050
v_video_socket_port = 9090
v_stream_image_quality = 20
v_controller_socket_ip = "192.168.137.198"
...
def receive_command(sock_cmd):
    try:
        ...
        command = int(data.decode('utf-8').strip())
...
def receive_speed(sock_speed):
    try:
        data, addr = sock_speed.recvfrom(256)
        speed = int(data.decode('utf-8').strip())
...
def init_sockets():
    ...
    print("Server ready for commands receival")
    
    sock_speed = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock_speed.bind(('0.0.0.0', v_speed_socket_port))
    sock_speed.settimeout(0)
    ...
    return sock_cmd, sock_video, sock_speed, sock_alert
...
def send_alert(alert):
    print(alert)
    try:
        sock_alert.sendto(alert.to_bytes(), (v_video_socket_ip, v_video_socket_port))         
    except Exception as e:
            print("Error sending alert:", e)

# Copy audios to internal memory
#for nome in ["turnon.wav", "turnoff.wav", "20class.kmodel"]:
for nome in ["20class.kmodel"]:
    copy_to_flash(nome)
...
sock_cmd, sock_video, sock_speed, sock_alert = init_sockets()
init_camera_lcd()
...
while v_continue == 1:
    ...
    v_command = receive_command(sock_cmd)
    
    v_speed = receive_speed(sock_speed)
    
    if v_command != v_command_not_received:
        v_alert = react_to_command(v_command, v_cx, v_cy)
        if v_alert != "":
           send_alert(v_alert)
        if v_command == v_turnoff_command:
            v_continue = 0
    else: 
